...
void Dump(IsoDirectoryRecord idr) {
	...
	writefln("  Extent:               %08X", cast(uint)idr.Extent);
	writefln("  Size:                 %08X", cast(uint)idr.Size);
	writefln("  Date:                 [...]");
...
class IsoEntry : FileContainer {
	...
	void writedr() {
		...
		if (udf_extent) {
			...
			udfs.position = 0x38;
			udfs.write(cast(uint)dr.Size);
			udfs.position = 0x134;
			udfs.write(cast(uint)dr.Size);
			udfs.write(cast(uint)((cast(uint)dr.Extent) - 262));
	...
	override int replace(Stream from, bool limited = true) {
		...
		//writefln("%d->%d", from.size, op.size);
		try {
			copy2From(op, from);
			//copyStream(from, op);
			...
			throw(new Exception(std.string.format("'%s': Can't write (%d -> %d) without removing limits (%d)", name, from.size, op.size, from.size - op.size)));
		}
	}
...
class IsoFile : IsoEntry {
	...
	override void makePPF(Stream to, long offset) {
		Stream s;
		ubyte[0xFF] temp;
		char[50] info; foreach (k, c; info) info[k] = ' ';
		to.writeString("PPF30");
		to.write(cast(ubyte)2);
		to.writeString(info);
		...
		long pos = (cast(long)cast(uint)dr.Extent) * 0x800;
		int size = cast(uint)dr.Size;
		
		//writefln("%08X    ", pos);
		
		s = new SliceStream(iso.stream, pos);
		
		while (size) {
			int csize = 0xFF;
			if (csize > size) csize = size;
			to.write(cast(long)pos);
			to.write(cast(ubyte)csize);
			s.read(temp[0..csize]);
			to.write(temp[0..csize]);
...
class Iso : IsoDirectory {
	...
	Iso copyIsoStructure(Stream s) {
		...
		copy2From(s, new SliceStream(stream, 0, (cast(ulong)firstDatasector) * 0x800));
		//writefln(firstDatasector);
	...
	void copyUnrecreatedFiles(Iso iso, bool show = true) {
		...
		foreach (ce; this) {			
			IsoEntry ie = cast(IsoEntry)ce;			
			if (cast(uint)ie.dr.Extent) continue;
	...
	void recreateFile(FileContainer ce) {
	...
	void recreateFile(FileContainer ce, char[] n, int addVoidSectors = 0) {
		Stream s = new File(n, FileMode.In);
		recreateFile(ce, s, addVoidSectors);
		s.close();
	}
	
	void recreateFile(FileContainer ce, Stream s, int addVoidSectors = 0) {
		s.position = 0;		
	...
	FileContainer oce; // OpenedFileContainer
	Stream writing;
	Stream startFileCreate(FileContainer ce) {
		oce = ce;
	...
	static uint sectors(uint733 size) {
		return sectors(cast(ulong)cast(uint)size);
	}
	
	void processFileDR(IsoDirectoryRecord dr) {
		uint ssect = cast(uint)dr.Extent;
		uint size  = cast(uint)dr.Size;
	...
	Stream openDirectoryRecord(IsoDirectoryRecord dr, bool limited = true) {
		ulong from = getSectorPos(dr.Extent);
		uint size  = cast(uint)dr.Size;
	...
	private ulong getSectorPos(uint733 sector) {
		return getSectorPos(cast(uint)sector);
	}
	...
	private void processDirectory(IsoDirectory id) {
		...
		uint maxPos = stream.position + cast(uint)bdr.Size;
		...
		while (true) {
			...
			if (dr.NameLength && name[0] != 0 && name[0] != 1) {
				...
				// Fichero
					...
					processFileDR(dr);
					if (cast(uint)dr.Extent < datastart) datastart = cast(uint)dr.Extent;
					IsoFile cif = new IsoFile();
					cif.drstream = drstream;
					cif.iso = this;
					cif.dr = dr;
					cif.size = cast(uint)dr.Size;
	...
	this(char[] s, bool readonly = false) {
		...
		//try {
			if (readonly) {
				...
				try {
					f = new File(s, FileMode.In | FileMode.Out);
				} catch (Exception e) {
					throw(new Exception(e.toString ~ " (for write)"));
				}
			}
			/*
		} catch (Exception e) {
	...
	override void regen(FileContainer fc, int delegate(char[], int, int, long, long, bool) callback = null) {
		uint min = 0xFFFFFFFF;
		foreach (_e; this) { IsoEntry e = cast(IsoEntry)_e;
			if (!e) continue;
			if (!e.name.length) continue;
			uint cpos = cast(uint)e.dr.Extent;
			if (min > cpos) min = cpos;
			//writefln("%s: %08X", e.name, cpos);
		}
		...
		foreach (ccp, _e; cchilds) { IsoEntry e = cast(IsoEntry)_e;
			if (!e) continue;
			if (!e.name.length) continue;
			//if (e.name == "AJI_D03.PKB") continue;
			if (callback && callback(e.name, ccp, cchilds.length, cast(long)stream.position, cast(long)stream.size, false)) break;
			if (!fc[e.name].exists) continue;
			try {
				Stream s = fc[e.name].open(FileMode.In); //scope(exit) { s.close(); }
				uint s_size = s.size;
				e.dr.Size = s_size;
				e.dr.Extent = cpos;
				stream.position = getSectorPos(cpos);
				copy2From(stream, s);
				cpos += sectors(s_size);
			} catch (Exception ex) {
				e.dr.Size = 0;
				if (callback && callback(e.name, ccp, cchilds.length, cast(long)stream.position, cast(long)stream.size, true)) break;
			}
			e.writedr();
		}
		//writefln();
