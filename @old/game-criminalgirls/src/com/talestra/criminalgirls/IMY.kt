...
object IMY : ImageFormat() {
	...
	fun read(s: SyncStream): Bitmap {
		...
		if (paletteSize != 0) {
			...
			val outBytes2 = if (swizzle) {
				//com.talestra.criminalgirls.PostProcessing.UnswizzleTexture(outBytes.sliceArray(actualWidth * height * 2 until outBytes.size) + outBytes.sliceArray(0 until actualWidth * height * 2), actualWidth, height, 16)
